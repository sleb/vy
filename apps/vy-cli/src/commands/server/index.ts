...
import { formatDuration, formatStatus, formatTimestamp } from "../../lib/formatters.js";
...
export async function start(options?: any): Promise<void> {
  ...
  try {
    ...
    const serverPath = join(process.cwd(), "packages/mcp-server-basic/dist/cli.js");
    ...
    if (options?.daemon) {
      ...
      spinner.succeed(`Server started in daemon mode (PID: ${serverProcess.pid})`);
 ...
 * Stop the MCP server
 */
export async function stop(options?: any): Promise<void> {
...
export async function status(options?: any): Promise<void> {
  ...
  try {
    ...
    const tableData = [
      ["Status", isRunning ? formatStatus("healthy", "Running") : formatStatus("unhealthy", "Stopped")],
      ["PID", pid ? String(pid) : chalk.gray("N/A")],
      ["Uptime", isRunning && pid ? formatDuration(getProcessUptime(pid)) : chalk.gray("N/A")]
    ];
    ...
    if (isRunning) {
      ...
      try {
        ...
        spinner.stop();
        tableData.push(["Connectivity", connected ? formatStatus("healthy", "Connected") : formatStatus("unhealthy", "Not responding")]);
      } catch {
        spinner.stop();
        tableData.push(["Connectivity", formatStatus("unhealthy", "Connection failed")]);
      }
    }
 ...
 */
export async function health(options?: any): Promise<void> {
...
export async function logs(options?: any): Promise<void> {
  try {
    if (!existsSync(SERVER_LOG_FILE)) {
      ...
      console.log(chalk.gray("   Logs are only available when running in daemon mode"));
      return;
    }
    ...
    if (options?.follow) {
      ...
      console.log(chalk.gray("   Use 'vy server logs' without --follow to view recent logs"));
...
function saveServerPid(pid: number): void {
  ...
  if (!existsSync(vyDir)) {
    require("fs").mkdirSync(vyDir, { recursive: true });
  }
...
function clearServerPid(): void {
  try {
    if (existsSync(SERVER_PID_FILE)) {
      require("fs").unlinkSync(SERVER_PID_FILE);
    }
  } catch {
 ...
 */
function getProcessUptime(pid: number): number {
  try {
    // This is a simplified uptime calculation
    // In a real implementation, you'd read from /proc/{pid}/stat on Linux
    return Date.now() - require("fs").statSync(SERVER_PID_FILE).mtimeMs;
  } catch {
    return 0;
  }
}
