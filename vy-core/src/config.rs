...
pub fn load_config(path: &Path) -> Result<VyConfig> {
    let config = Config::builder()
        .add_source(config::File::from(path))
        .add_source(config::Environment::with_prefix("VY"))
        .build()?
        .try_deserialize::<VyConfig>()
        .with_context(|| {
            format!(
                "Failed to deserialize config from file at {}",
                path.display()
            )
        })?;
...
pub fn load_or_create_config(path: &Path) -> Result<VyConfig> {
...
/// Save Vy configuration to a file
pub fn save_config(config: &VyConfig, path: &Path) -> Result<()> {
    if let Some(parent_dir) = path.parent() {
        fs::create_dir_all(parent_dir).with_context(|| {
            format!(
                "Failed to create parent directories for config file at {}",
                path.display()
            )
        })?;
    }
    ...
    fs::write(path, toml_string)
        .with_context(|| format!("Failed to write config to file at {}", path.display()))?;
